function World5(game) {
	...
	this.create = function () {
		...
		for (idx in monsterData) {
			if (idx <= 1 && !monsterData[idx][8] && monsterData[idx][9] == 5) {
			...
			} else if (!monsterData[idx][8] && monsterData[idx][9] == 5) {
		...
		//加载地图
		var map = game.add.tilemap('world-5');
		...
		map.setCollision([5163]);//仅指定的tileid执行碰撞检测
		//切換到world2
		map.setTileLocationCallback(8, 32, 1, 1, this.changeworld2, this);
		...
		game.add.sprite(0, 0, 'world5-ground'); //用一张大图代替tilemaplayer可提高性能
		...
		game.add.sprite(0, 0, 'world5-fore');
