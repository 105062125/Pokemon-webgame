function World3(game) {
	...
	this.create = function () {
		...
		for (idx in monsterData) {
			if (idx <= 1 && !monsterData[idx][8] && monsterData[idx][9] == 3) {
			...
			} else if (!monsterData[idx][8] && monsterData[idx][9] == 3) {
		...
		//加载地图
		var map = game.add.tilemap('world-3');
		...
		map.setCollision([2319]);//仅指定的tileid执行碰撞检测
		//切換到world1
		map.setTileLocationCallback(5, 16, 1, 1, this.changeworld1, this);
		map.setTileLocationCallback(57, 5, 1, 1, this.changeworld4, this);
		...
		game.add.sprite(0, 0, 'world3-ground'); //用一张大图代替tilemaplayer可提高性能
