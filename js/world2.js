function World2(game) {
	...
	this.create = function () {
		...
		for (idx in monsterData) {
			if (idx <= 1 && !monsterData[idx][8] && monsterData[idx][9] == 2) {
			...
			} else if (!monsterData[idx][8] && monsterData[idx][9] == 2) {
		...
		//加载地图
		var map = game.add.tilemap('world-2');
		...
		//切換到world1
		map.setTileLocationCallback(15, 1, 1, 1, this.changeworld1, this);
		map.setTileLocationCallback(16, 31, 1, 1, this.changeworld5, this);
		...
		game.add.sprite(0, 0, 'world2-ground'); //用一张大图代替tilemaplayer可提高性能
